...
--- created stored procedure silver.load_silver to automatically truncate and load new data
...
IF OBJECT_ID ('gold.dim_call_reason','U') IS NOT NULL
    DROP TABLE gold.dim_call_reason
...
IF OBJECT_ID ('gold.fact_calls','U') IS NOT NULL
    DROP TABLE gold.fact_calls
...
-->> INSERT data into gold tables (expecting business ready data with business logics)
--- created stored procedure gold.load_gold to automatically truncate and load new data
...
BEGIN
    BEGIN TRY
        ...
        insert into gold.dim_customer (customer_id,customer_name,date_of_birth,customer_age,customer_age_group,gender,
                    city,[state], region,customer_since,customer_loyalty_band,customer_segment,[product],
        ...
        TRUNCATE TABLE gold.fact_calls
